...
```bash
helm upgrade --install dify-test ./dify-3.11.1-suse.2.tgz \
  --namespace dify \
  --create-namespace \
...
`3.11.1-suse.2` 表示针对 SUSE Rancher UI 和同集群第二套 Dify 安装的适配版本，不改变 Dify Enterprise 应用版本。Dify Enterprise 许可、镜像和应用支持仍应遵循 Dify 的授权与支持条款。
